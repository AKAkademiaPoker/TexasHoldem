...
public class ResultSearcher {
	...
	private void setBestCardsAndHandType(Player player, HandTypeSearcher searcher) {
		...
		getBestCardInHand(player);
	}

	private void getBestCardInHand(Player player) {
		List<Card> cardsInHand = new ArrayList<>();
		cardsInHand.add(player.getCard1());
		cardsInHand.add(player.getCard2());
		cardsInHand.removeAll(player.getBestCards());
		HandTypeSearcher.sortCardsByRank(cardsInHand);
		List<Card> tempCards = player.getBestCards();
		tempCards.addAll(cardsInHand);
		if (tempCards.size() > 5) {
			tempCards.subList(5, tempCards.size()).clear();
		}
		
		player.setBestCards(tempCards);
	}
